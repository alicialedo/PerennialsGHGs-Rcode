...
# it is independent to account for grafting and cases where the AGB is replaced but roots no  
  ...
  fineroot<-((2.73*BGBaux^(-0.841))/100)*BGBaux #not addedd to biomass  BUG CORRECTED
  #fineroot<-((2.73*Nyear^(-0.841))/100)*BGBaux #not addedd to biomass  
...
if (com_chip_prun=="YES"){
  ...
  if(enclosed_comp=="YES") {
    CO2[,7]<- ((indbio_annual[,4]*Cwood*(1-(55/100))*-44/12) #bug corrected
...
if (com_litter=="YES"){
  ...
  if(enclosed_comp=="YES") {
    CO2[,10]<- ((indbio_annual[,7]*Cleaf*(1-(55/100))*-44/12)+ #bug corrected
...
if (comp_fruit=="YES"){
  ...
  if (open_compost=="YES"){
    ...
    if(enclosed_comp=="YES") {
      CO2[,12]<- ((indbio_annual[,8]*(Perbad/100)*Cfruit*(1-(55/100))*-44/12)+ #bug corrected
...
if (comp_pulp=="YES"){      
  if (open_compost=="YES"){
    ...
    if(enclosed_comp=="YES") {
      CO2[,14]<- (indbio_annual[,9]*(1-(Perbad/100))*Cfruit*(1-(55/100))*-44/12)+ #bug corrected
...
if(PartFruitYield!="All")  {
  ...
  if (comp_seed=="YES"){      
    if (open_compost=="YES"){
      ...
      if(enclosed_comp=="YES") {
        CO2[,14]<- (indbio_annual[,10]*(1-(Perbad/100))*Cfruit*(1-(55/100))*-44/12)+ #bug corrected
...
#End FINAL CUT - Residuals from the tree, in Kg per tree per year
###################################################
...
#???TREES THAT DIE DURING THE CROP CYCLE
#auxiliar dead trees# warning, this is not for a single tree, this is an auxiliar field that will be need
#I put a small amount of dead trees on the tree, mathematical artefact
#I assume a constant mortality rate
...
#the residues are burnt
if (burn_dead=="YES"){
  CO2[,17]<- ((indbio[,5]*(PERburn_dead/100)*1.509)-
  (indbio[,5]*(PERburn_dead/100)*Cwood*44/12))*((PNdie/100)/Nyear) #CO2 in kg per tree
    N2O[,17]<- indbio[,5]*(PERburn_dead/100)*0.00038*((PNdie/100)/Nyear)   #NO2 in Kg
  CH4[,17]<- indbio[,5]*(PERburn_dead/100)*0.00568*((PNdie/100)/Nyear) #methane in Kg    
}
...
if (soil_chip_dead=="YES"){
  ...
  for (j in 1:Nyear){
    for (i in 1:Nyear){
      deadaux[i]<-indbio[j,5]*(exp(-1*(k_chip)*i))*((PNdie/100)/Nyear)  
...
# actual number of trees end of the period
ifelse (gaping=="YES", Ntree<-Nind, Ntree<-as.integer(Nind-((Nind*PNdie/100)/2)))
#WARNING: WE DON´T KNOW WHEN THE TREES EXACTLY DIE, BUT THIS IS AN APPROXIMATION-IT ASSUMES TREE MORTALITY RATE IS CONSTANT


  #CO2 Kg per year
  farmCO2_annual<- CO2*Nind*Area 
  farmCO2_annual[,1]<-c(1:Nyear)  
    #N2O Kg per year
  farmN2O_annual<- N2O*Nind*Area 
  farmN2O_annual[,1]<-c(1:Nyear)   
  #CH4 Kg per year
  farmCH4_annual<- CH4*Nind*Area
  farmCH4_annual[,1]<-c(1:Nyear)     
  #CO2eq Kg per year
  farmCO2eq_annual<- CO2eq*Nind*Area 
